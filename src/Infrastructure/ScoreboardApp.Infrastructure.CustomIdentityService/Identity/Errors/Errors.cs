...
namespace ScoreboardApp.Infrastructure.CustomIdentityService.Identity.Errors
{
    ...
    {
        public static Error UserNotFoundError(string username) => new() { Code = "user.not_found", Message = $"User with given username ({username}) does not exist." };
        ...
        public static Error UserAlreadyExistsError(string username) => new() { Code = "user.already_exists", Message = $"User with given username ({username}) already exists." };
